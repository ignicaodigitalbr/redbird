...
var
  ...
  validUrl = require('valid-url-accent'),
...
function ReverseProxy(opts) {
  ...
  if (opts.logs) {
    ...
    var accessLogFilename = path.join(opts.logs.path, opts.logs.access);
    var errorLogFilename = path.join(opts.logs.path, opts.logs.error);

    if (opts.logs.error) {
      var errorLogFile = new winston.transports.File({
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    cluster.on('exit', function (worker, code, signal) {
      ...
      log && log.error('code: ' + code + 'signal: ' + signal + ' worker died un-expectedly... restarting it.');
      cluster.fork();
    });
  } else {
    ...
    log && log.info('Started a Redbird reverse proxy server on port ' + opts.port);
  }
  ...
  function websocketsUpgrade(req, socket, head) {
    ...
    _this._getTarget(src, req).then(function (target) {
      ...
      if (target) {
        ...
        log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 178 - src:' + src + ' target:' + target);
...
ReverseProxy.prototype.findInDatabase = function (src) {
  var _this = this;
	var domain = src;
	var otherDomain = 'www.' + src;

	if (domain.indexOf('www') !== -1) {
		otherDomain = domain.replace('www.', '');
	}
  ...
  return _this.model.findOne({
    ...
    .then(function (item) {
      if (!item || !item.dataValues) {
        log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 223 - dominio ' + src + ' não existe no banco de dados');
        throw new ReferenceError(`No domain ${src}`);
      }

      const domain = item.dataValues;

      if (!_this.addDomain) {
        log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 229 - Método _this.addDomain não encontrado');
        throw new ReferenceError(`No addDomain method`);
      }

      _this.addDomain(domain);
    })
};

function redirect(certs, src, target, instance, req, res, ssl, log) {
  if (shouldRedirectToHttps(certs, src, target, instance)) {
    return redirectToHttps(req, res, target, ssl, log);
  } else {
    return instance.proxy.web(req, res, { target: target });
  }
};
...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, log, opts) {
  ...
  var server = this.server = httpServerModule.createServer(function (req, res) {
    ...
    _this._getTarget(src, req).bind(_this).then(function (target) {
      accessLogger(req, res, function (err) {
        if (!target) {
          if (!_this.model || !_this.addDomain || !_this.target) {
            log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 257 - No params for search in database. Model: ' + _this.model + ' addDomain: ' + _this.addDomain + ' target: ' + _this.target);
            return respondNotFound(req, res);
          }

          _this.findInDatabase(src).then(function () {
            var buildedTarget = ReverseProxy.buildTarget(_this.target, opts);
            return redirect(_this.certs, src, buildedTarget, _this, req, res, opts.ssl, log);
          }).catch(function (error) {
						if (log) {
							try {
								var parsedError = JSON.stringify(error);
								log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 268 - src:' + src + ' target:' + target + ' - error: ' + parsedError);
							} catch(e) {
								...
								log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 276 - src:' + src + ' target:' + target + ' - error: ' + errorMessage);
							}
						}
        ...
        return redirect(_this.certs, src, target, _this, req, res, opts.ssl, log);
      });
    });
  });
  //
  ...
  server.on('error', function (err) {
    log && log.error('Server Error: ' + err);
  });
...
ReverseProxy.prototype.setupHttpsProxy = function (proxy, websocketsUpgrade, log, sslOpts){
  ...
  var httpsServer = this.httpsServer = https.createServer(ssl, function (req, res) {
    ...
    _this._getTarget(src, req).then(function (target) {
      accessLogger(req, res, function (err) {
        if (!target) {
          if (!_this.model || !_this.addDomain || !_this.target) {
            log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 350 - No params for search in database');
            return respondNotFound(req, res);
          }

          _this.findInDatabase(src).then(function () {
            return proxy.web(req, res, { target: _this.target });
          }).catch(function (error) {
            log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 356 - src:' + src + ' target:' + target);
            return respondNotFound(req, res);
          });
        }
  ...
  httpsServer.on('error', function (err) {
    log && log.error('HTTPS Server Error: ' + err);
  });

  httpsServer.on('clientError', function (err) {
    log && log.error('HTTPS Client  Error: ' + err);
  });
...
ReverseProxy.prototype.updateCertificates = function (domain, email, production, renewWithin, renew) {
  ...
  return letsencrypt.getCertificates(domain, email, production, renew, this.log).then(function (certs) {
    if (certs) {
      ...
      _this.log && _this.log.info('Renewal of ' + domain + ' in ' + Math.floor(renewTime / ONE_DAY) + ' days');

      function renewCertificate() {
        _this.log && _this.log.info('Renewing letscrypt certificates for ' + domain);
      ...
      //
      _this.log && _this.log.info('Could not get any certs for ' + domain);
    }
  }, function (err) {
    _this.log.error('Error getting LetsEncrypt certificates: ' + err);
  });
};
...
ReverseProxy.prototype._getTarget = function (src, req) {
  ...
  return this.resolve(src, url, req).bind(this).then(function (route) {
    if (!route) {
      this.log && this.log.error('src: ' + src + ' url: ' + url + ' no valid route found for given source');
      return;
    }
    ...
    this.log && this.log.info('Proxying ' + src + url + ' to ' + path.join(target.host, req.url));
...
ReverseProxy.prototype.bindAddDomain = function (method) {
  this.addDomain = method;
};
...
function redirectToHttps(req, res, target, ssl, log) {
  ...
  log && log.info('Redirecting ' + path.join(req.headers.host, req.url) + ' to ' + url);
  //
...
function createCredentialContext(key, cert, ca) {
  ...
  if (isValidPath(key) && isValidPath(cert)) {
    opts.key = getCertData(key);
    opts.cert = getCertData(cert);
    if (ca && isValidPath(ca)) {
