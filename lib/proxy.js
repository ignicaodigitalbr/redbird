  ...
  function websocketsUpgrade(req, socket, head) {
    ...
    _this._getTarget(src, req).then(function (target) {
      ...
      if (target) {
        ...
        log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 175 - src:' + src + ' target:' + target);
    ...
    _this._getTarget(src, req).bind(_this).then(function (target) {
      accessLogger(req, res, function (err) {
        if (target){
          ...
          log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 221 - src:' + src + ' target:' + target);
    ...
    _this._getTarget(src, req).then(function (target) {
      accessLogger(req, res, function (err) {    
        if (target) {
          ...
          log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 311 - src:' + src + ' target:' + target);
          respondNotFound(req, res);
        }
      });
    });
