...
function redirect(certs, src, target, instance, req, res, ssl, log) {
  if (shouldRedirectToHttps(certs, src, target, instance)) {
    redirectToHttps(req, res, target, ssl, log);
  } else {
    instance.proxy.web(req, res, { target: target });
  }
};
      ...
      accessLogger(req, res, function (err) {
        if (!target) {
          ...
          _this.findInDatabase(src).then(function () {
            redirect(_this.certs, src, _this.target, _this, req, res, opts.ssl, log);
        ...
        redirect(_this.certs, src, target, _this, req, res, opts.ssl, log);
      });
    });
  });
  //
