  ...
  function websocketsUpgrade(req, socket, head) {
    ...
    _this._getTarget(src, req).then(function (target) {
      ...
      if (target) {
        ...
        log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 174 - src:' + src + ' target:' + target);
    ...
    _this._getTarget(src, req).bind(_this).then(function (target) {
      accessLogger(req, res, function (err) {
        if (target){
          ...
          log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 225 - src:' + src + ' target:' + target);
    ...
    _this._getTarget(src, req).then(function (target) {
      accessLogger(req, res, function (err) {    
        if (target) {
          ...
          log && log.error(Date.now().toLocaleString() + ' - NOT FOUND - line 320 - src:' + src + ' target:' + target);
          respondNotFound(req, res);
        }
      });
    });
